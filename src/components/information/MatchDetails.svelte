...
<div transition:slide={{ duration: 300, easing: cubicOut }} class="border-t-neutral-600/[0.25] border-t-2 grid border-solid gap-2 p-2 w-full">
  <div class="items-center flex text-sm font-medium">
    <button class={`items-center border-b-neutral-600/[0.25] rounded-bl-md border-b-2 border-l-neutral-600/[0.25] border-l-2 border-t-neutral-600/[0.25] rounded-tl-md border-t-2 cursor-pointer flex-grow justify-center py-1 px-3 text-center flex w-32 h-9 text-zinc-100 border-r-2 border-r-neutral-600/[0.25] rounded-tr-md rounded-br-md ${selectedMatchTab === 'scoreboard' ? 'bg-[#e6b824]' : 'bg-zinc-800'}`} on:click={() => selectedMatchTab = 'scoreboard'}><span>Scoreboard</span></button>
  </div>
                ...
                <div class="w-full sm:w-1/3 flex items-center justify-start mb-2 sm:mb-0">
                  <a href={`/players/${player.id}`} class="flex items-center gap-2">
                    <div class="bg-{teamIndex === 0 ? 'teal' : 'red'}-400 flex rounded-md">
                      <img src={`/images/sponsor-logos/${player.sponsor_name}.png`} alt={player.sponsor_name} class="w-6 h-6 sm:w-8 sm:h-8 border-2 border-{teamIndex === 0 ? 'teal' : 'red'}-400 border-solid rounded-md" />
                    </div>
                    <div class="flex">
                    ...
                    <p class="text-gray-200 text-xs sm:text-sm truncate max-w-[100px] sm:max-w-[150px]">{player.name}</p>
                  </a>
                </div>
